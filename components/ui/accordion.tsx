...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-7 text-left text-lg font-medium tracking-[-0.01em] transition-all hover:text-primary/90 [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-8 pt-1 leading-relaxed", className)}>{children}</div>
  </AccordionPrimitive.Content>
))
