...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-6 top-6 z-10 rounded-full bg-white/90 p-2 text-foreground/70 ring-offset-background transition-all hover:bg-white hover:text-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2">
        <X className="h-5 w-5" />
...
const DialogHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col space-y-1.5 text-center sm:text-left p-8 pb-4",
      className
    )}
    {...props}
  />
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      "flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 p-8 pt-0",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn(
      "text-3xl font-display tracking-[-0.02em] leading-none",
      className
    )}
    {...props}
  />
